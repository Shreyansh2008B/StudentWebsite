...
function Card({ text, desc, background, width, height ,textcolor ,desccolor}) {
  return (
    <div 
    ...
    >
      <h3 className="font-bold text-lg text-black" style={{color:textcolor}}>{text}</h3>
      {desc && <p className="text-sm text-black leading-snug"  style={{color:desccolor}}>{desc}</p>}
    </div>
  );
}
